...
authRouter.post("/register", async(request, response) => {
    try {
        ...
        const existingUser = users.find((userDB) => {
            return userDB.email.toLowerCase() === email.toLowerCase();
        });
        ...
        response.status(500).send({
            message: "Unexpected error while trying to create account",
            error: error,
        });
    }
});
...
authRouter.post("/login", async(request, response) => {
    try {
        ...
        response.status(500).send({
            message: "Unexpected error while trying to login",
            error: error,
        });
    }
});
